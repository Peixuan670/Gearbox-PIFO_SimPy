        ...
        while True:
            ...
            if not pkt == 0:
                if pkt.get_finish_time() < self.pifo_max_time:
                    ...
                    enque_fifo_index = get_enque_fifo(pkt.get_finish_time())
                    # enque FIFO
        ...
        while True:
            ...
            if not pkt == 0:
                if pkt.get_finish_time() < self.pifo_max_time:
                    ...
                    self.fifo_w_in_pipe_arr[enque_fifo_index].put(popped_pkt)
                    yield self.fifo_w_out_pipe_arr[enque_fifo_index].get()
                    print("[Level] mig: pkt {} recycled to fifo {}".format(pkt.get_uid(), enque_fifo_index))
        ...
        while True:
            ...
            if self.rld_pipe_dat is not None:
                while pkt_reloaded < pkt_to_reload:
                    ...
                    # recycle popped_pkt FIFO
                    self.fifo_w_in_pipe_arr[index].put(popped_pkt)
                    yield self.fifo_w_out_pipe_arr[index].get()
                    print("[Level] during reloading: pkt {} recycled to fifo {}".format(pkt.get_uid(), index))
                if self.pifo.get_len() < self.pifo_threshold:
