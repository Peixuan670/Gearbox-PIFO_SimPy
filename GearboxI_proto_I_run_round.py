...
class Gearbox_I(HW_sim_object):
        ...
        while True:
            ...
            if insert_level = -1:
                ...
                if (self.level_ping_pong_arr[index] == True):
                    ...
                    current_fifo_index = self.levelsA[index].cur_fifo
                else:
                    # Current serving set B
                    current_fifo_index = self.levelsB[index].cur_fifo
                
                enque_current_set = ((current_fifo_index + fifo_index_offset) < self.fifo_num_list[index])
                #enque_current_set = self.enque_current_set(insert_level, pkt_finish_time)

                if (((self.level_ping_pong_arr[index] == True) and enque_current_set)):
                    # Case 01: Enque current Set A
                    enque_index = current_fifo_index + fifo_index_offset
                    self.enq_pipe_cmd_arr_A[index].put((pkt_des, enque_index))
                    (popped_pkt_valid, popped_pkt) = yield self.enq_pipe_sts_arr_A[index].get()
                elif (((self.level_ping_pong_arr[index] == False) and enque_current_set)):
                    # Case 02: Enque current Set B
                    enque_index = current_fifo_index + fifo_index_offset
                    self.enq_pipe_cmd_arr_B[index].put((pkt_des, enque_index))
                    (popped_pkt_valid, popped_pkt) = yield self.enq_pipe_sts_arr_B[index].get()
                elif (((self.level_ping_pong_arr[index] == True) and not enque_current_set)):
    ...
    def deque_p(self):
        while True:
            ...
            deque_level_index = find_deque_level()
    ...
    def enque_current_set(self, level_index, fifo_index_offset):
        if self.level_ping_pong_arr[level_index]:
            ...
            set_max_vc = (math.floor(self.vc/self.granularity_list[level_index]) + (self.fifo_num_list[level_index] - levels_A[level_index].cur_fifo)) * self.granularity_list[level_index]
            if finish_time < set_max_vc: # TODO: < or <=
                return True
            else:
                return False
        else:
            set_max_vc = (math.floor(self.vc/self.granularity_list[level_index]) + (self.fifo_num_list[level_index] - levels_B[level_index].cur_fifo)) * self.granularity_list[level_index]
            if finish_time < set_max_vc: # TODO: < or <=
                return True
            else:
                return False    
